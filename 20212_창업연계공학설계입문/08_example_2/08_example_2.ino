...
// turn on the LED if the distance is between dist_min and dist_max
  if(dist_raw < dist_min || dist_raw > dist_max) {
    ...
    if(dist_raw < 200)  analogWrite(PIN_LED, (int)(-2.55*dist_raw+510));
    else  analogWrite(PIN_LED, (int)(2.55*dist_raw-510));
  }
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if(reading < dist_min || reading > dist_max) reading = dist_raw; // return dist_raw when out of range.
  return reading;
